...
type SocialProvider = "google" | "microsoft-entra-id" | "facebook" | "instagram";
...
const providerConfig = {
  ...
  facebook: {
    ...
    color: "hover:bg-blue-600/10 border-blue-600/20",
  },
  instagram: {
    name: "Instagram",
    icon: (
    ...
    color: "hover:bg-pink-500/10 border-pink-500/20",
  },
};

export function SocialLoginButton({ provider, callbackUrl }: SocialLoginButtonProps) {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`flex w-full items-center justify-center gap-3 rounded-full border ${config.color} bg-white/5 px-4 py-3 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-60`}
    >
      {isLoading ? (
        <>
...
export function SocialLoginButtons({ callbackUrl }: { callbackUrl: string }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <SocialLoginButton provider="microsoft-entra-id" callbackUrl={callbackUrl} />
      <SocialLoginButton provider="facebook" callbackUrl={callbackUrl} />
      <SocialLoginButton provider="instagram" callbackUrl={callbackUrl} />
    </div>
  );
}
