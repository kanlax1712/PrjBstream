...
export function MobileNav({ session }: MobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-slate-950/95 backdrop-blur-md mobile-nav-safe-bottom md:hidden">
        ...
        {mobileNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`channel-button-ripple flex flex-col items-center justify-center gap-1 rounded-xl px-3 py-2 min-h-[44px] text-xs transition ${
                isActive
                  ? "text-cyan-400"
                  : "text-white/60"
              }`}
